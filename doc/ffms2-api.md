...
```c++
int64_t FFMS_GetContainerFirstTimeI(FFMS_Indexer *Indexer);
```
Returns the container start time of the media file represented by the given `FFMS_Indexer` object.
The value is expressed in `AV_TIME_BASE` units (1,000,000).
This value corresponds to `AVFormatContext::start_time` in FFmpeg.
